...
//import { westoneDllIO } from '../../utilities/dll-io'
import { getLogDir } from '../../utilities/Utils'
...
export default {
  ...
  methods: {
    ...
    // 点击发送按钮发送信息
    sendMessage() {
      //log.info('sendMessage : dll path =' + electron.remote.app.getAppPath() + '\\static\\dll\\zzb_clipboard.dll');
      if (this.content.length <= 1) {
        ...
        XmppClient.getInstance().sendMessage(
          'yangjing@mxt.westone.com/sim',
          this.content,
          'chat'
        )
        //westoneDllIO.copyTextToClipboard(this.content, getLogDir())
        const sendMsg = {
          content: this.content,
          reply: this.reply,
        }
        const db = new Dexie('MxtDatabase')
        db.version(1).stores({
